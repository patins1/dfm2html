...
type
  TTntListColumn = TListColumn;
  TTntListColumns = TListColumns;
  TTntListItem = TListItem;
  TTntListItems = TListItems;
  TTntCustomListView = class(TCustomListView);
  TTntListView = class(TListView);
  TTntToolButton = class(TToolButton);
  TTntToolBar = class(TToolBar);
  TTntCustomRichEdit = class(TCustomRichEdit);
  TTntRichEdit = class(TRichEdit);
  TTntCustomTabControl = class(TCustomTabControl);
  TTntTabControl = class(TTabControl);
  TTntTabSheet = class(TTabSheet);
  TTntPageControl = class(TPageControl);
  TTntTrackBar = class(TTrackBar);
  TTntProgressBar = class(TProgressBar);
  TTntCustomUpDown = class(TCustomUpDown);
  TTntUpDown = class(TUpDown);
  TTntDateTimePicker = class(TDateTimePicker);
  TTntMonthCalendar = class(TMonthCalendar);
  TTntPageScroller = class(TPageScroller);
  TTntStatusPanel = TStatusPanel;
  TTntStatusPanels = TStatusPanels;
  TTntCustomStatusBar = class(TCustomStatusBar);
  TTntStatusBar = class(TStatusBar);
  TTntTreeNode = TTreeNode;
  TTntTreeNodeClass = class of TTntTreeNode;
  TTntTreeNodes = TTreeNodes;
  TTntCustomTreeView = class(TCustomTreeView);
  TTntTreeView = class(TTreeView);
...
type
  ...
  TTNTNMTBGETINFOTIPW = packed record
    hdr: TNMHdr;
    dwFlags: DWORD;
    pszText: PWideChar;
    cchTextMax: Integer;
  ...
  PTNTNMTBGETINFOTIPW = ^TTNTNMTBGETINFOTIPW;
...
{TNT-WARN TCustomListView}
  ...
  TTntInfoTipWideEvent = procedure(Sender: TObject; Item: TTNTListItem; var InfoTip: widestring) of object;
  ...
  private
    ...
    FOnInfoTipWide: TTntInfoTipWideEvent;
  ...
  protected
    ...
    property OnInfoTipWide: TTntInfoTipWideEvent read FOnInfoTipWide write FOnInfoTipWide;
  public
...
{TNT-WARN TToolBar}
  ...
  private
    TB_ADDSTRING_Pending: Boolean;
    FCaption: WideString;
    procedure TBAddStringA(var Message: TMessage); message TB_ADDSTRINGA;
    procedure TBSetButtonInfoA(var Message: TMessage); message TB_SETBUTTONINFOA;
  ...
  if (not Win32PlatformIsUnicode) then
    ...
    begin
      case NMHdr^.code of
        ...
        LVN_GETINFOTIPW:
          begin
            if Assigned(FOnInfoTipWide) then
            begin
              Item := Items[PTNTNMTBGetInfoTipW(NMHdr).iItem];
              InfoTipWide := PTNTNMTBGetInfoTipW(NMHdr).pszText;
              FOnInfoTipWide(Self, Item, InfoTipWide);
              WStrLCopy(PTNTNMTBGetInfoTipW(NMHdr).pszText,pwidechar(InfoTipWide),wstrlen(pwidechar(InfoTipWide)));
            end
            else
            begin
...
constructor TTntToolBar.Create(AOwner: TComponent);
begin
  inherited;

  TB_ADDSTRING_Pending := False;
end;
...
begin
  ...
  if Win32PlatformIsUnicode then
    SendMessage(Handle, TB_SETUNICODEFORMAT, 1, 0);
end;
...
procedure TTntToolBar.TBAddStringA(var Message: TMessage);
begin
  if not Win32PlatformIsUnicode then
    inherited
  else
    TB_ADDSTRING_Pending := True;
end;

procedure TTntToolBar.TBSetButtonInfoA(var Message: TMessage);
begin
  if not Win32PlatformIsUnicode or not TB_ADDSTRING_Pending then
    inherited
  else begin
    PTBButton(Message.LParam).iString := SendMessage(Handle, TB_ADDSTRINGW, 0, Integer(PWideChar(WideString(TTntToolButton(Buttons[Message.WParam]).Caption + WideChar(#0)))));

    Message.Result := SendMessage(Handle, TB_SETBUTTONINFOW, Message.WParam, Message.LParam);
...
begin
  if not Win32PlatformIsUnicode 
    or not TB_ADDSTRING_Pending 
    or (PTBButton(Message.LParam).iString = -1) 
    or not (Buttons[Message.WParam] is TTntToolButton) then
    inherited
  else 
  begin
    PTBButton(Message.LParam).iString := SendMessage(Handle, TB_ADDSTRINGW, 0, Integer(PWideChar(WideString(TTntToolButton(Buttons[Message.WParam]).Caption + WideChar(#0)))));

    Message.Result := SendMessage(Handle, TB_INSERTBUTTONW, Message.WParam, Message.LParam);
