...
{$ENDIF}
  WSocket;
type TLongInt=Int64;//°!°
const OwnError_EmptyMessage=1;//°!°
      OwnError_EmptyMessageStr='Error! Connection terminated by server';
...
type
  ...
  protected
    ...
    procedure   DisplayLastResponse; virtual;
  ...
  public
    ...
    property    SizeResult      : TLongInt                read  FSizeResult;
    ...
    property    ByteCount       : TLongInt                read  FByteCount;
    ...
    property ResumeAt           : TLongInt                read  FResumeAt
                                                         write FResumeAt;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function GetInteger(Data : PChar; var Number : TLongInt) : PChar; overload;
var
    bSign : Boolean;
begin
    Number := 0;
...
function GetInteger(Data : PChar; var Number : LongInt) : PChar; overload;
var i64:TLongInt;
begin
 result:=GetInteger(Data,i64);
 Number:=i64;
end;
...
var
    Len     : Integer;
    Buffer  : array [1..{4096°!°}8192] of Char;  { Should use a dynamic buffer instead... }
    aSocket : TWSocket;
    I, J    : Integer;
    Line    : String;
begin
...
begin
    ...
    if FPassive and (Error=0) then //°!°
    begin
    //Put code at the end of DoGetAsync to this location
    StateChange(ftpWaitingResponse);
    FNext := Next1GetAsync;
    ...
    if Error <> 0 then begin
        ...
        SetErrorMessage;
        FDataSocket.Close;
        FRequestResult := {°!°FStatusCode}Error;
        TriggerRequestDone(FRequestResult);
    end;
end;
...
begin
    if Error <> 0 then begin
        ...
        FStatusCode    := 500;
        FRequestResult :=  {°!°FStatusCode}Error;    { 21/05/99 }
        SetErrorMessage;
...
begin
    ...
    { to have received the s erver banner.                                }
    if Error <> 0 then begin
        FLastResponse  := '500 ' + WSocketErrorDesc(Error) +
                          ' (Winsock error #' + IntToStr(Error) + ')';
        FStatusCode    := 500;
        FRequestResult := {°!°FStatusCode}Error;  { Heedong Lim, 05/14/1999 }
...
begin
    ...
    if (Len = 0) and not (FFctPrv in [ftpFctAbortXfer,ftpFctQuit])  {(FState=ftpWaitingBanner)} then begin{°!°}
                FLastResponse := OwnError_EmptyMessageStr;{'Error! Received empty response from server'};
                FStatusCode   := OwnError_EmptyMessage;
                SetErrorMessage;
                FRequestResult := FStatusCode;
//                TriggerRequestDone(FRequestResult); <- wird in FControlSocket.Close aufgerufen
                FControlSocket.Close;

    {
Fall tritt ein, wenn der ControlSocket niemals irgendwelche Daten empfängt.
Da FlashGet das als "Time out." interpretiert, tun wir das auch.
Ansonsten wird HandleError('FTP component not connected') bei
FNextRequest; in TriggerRequestDone ausgelöst!
}
//        FControlSocket.InternalClose(TRUE, WSAETIMEDOUT);
//         FControlSocket.DataAvailableError(WSAETIMEDOUT, 'data receive error');
        Exit;
    end;

    if Len = 0 then begin
        TriggerDisplay('! Received 0 bytes from server'); //°!°
        FControlSocket.Close;
        Exit;
    end;
    if Len < 0 then
    begin
        //{kommt z.B. auch während banner-receive vor} TriggerDisplay('DEB received -1 at ftp dataavailable'); //°!°
        Exit;
    end;

(*        if (FState = ftpWaitingBanner) and (FReceiveLen=0) then begin {°!°}
    ...
    if not (FRequestType in [ftpRqAbort]) then begin
        if Error <> 0 then begin
            FLastResponse  := '500 Control connection closed. ' +
                              ...
                              ' (Winsock error #' + IntToStr(Error) + ')';
            FStatusCode    := 500;
            FRequestResult :=  {°!°FStatusCode}Error;    { 06 apr 2002 }
            SetErrorMessage;
        end;
        TriggerRequestDone(FRequestResult);
    end;
