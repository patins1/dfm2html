...
function WideShortCutToText(WordShortCut: Word): WideString;
   ...
   function GetShiftKeyText(VirtualKey: UINT): WideString;
   var
     Len: Integer;
     KeyText: array[0 .. 31] of WideChar;
   begin
     Len := GetKeyNameTextW((VirtualKey and $1FF) shl 16, KeyText,
       SizeOf(KeyText) div SizeOf(KeyText[0]));
     SetString(Result, KeyText, Len);
     Result := copy(Result, 1, 1) + copy(WideLowerCase(Result), 2, Len - 1);
     if (Pos('(', Result) > 0) then
       Result := copy(Result, 1, Pos('(', Result))
         + WideUpperCase(copy(Result, Pos('(', Result) + 1, 1))
         + copy(Result, Pos('(', Result) + 2, Len - Pos('(', Result) - 1);
   end;

begin
   case WordShortCut and $FF of
     $20..$28, {Misc Nav}
     $2D..$2E: {INS, DEL}
       Result := GetShiftKeyText(MapVirtualKeyW(WordShortCut and $FF, 0) + $100);
     $08..$09, {BKSP, TAB}
     $1B, {ESC}
     $0D, {ENTER}
     $70..$87: {F1 - F24}
       Result := GetShiftKeyText(MapVirtualKeyW(WordShortCut and $FF, 0));
     else
       Result := WideGetSpecialName(WordShortCut and $FF);
   end;
   if WordShortCut and scShift <> 0 then
     Result := GetShiftKeyText(MapVirtualKeyW(VK_SHIFT and $FF, 0)) + '+' + Result;
   if WordShortCut and scAlt <> 0 then
     Result := GetShiftKeyText(MapVirtualKeyW(VK_MENU and $FF, 0)) + '+' + Result;
   if WordShortCut and scCtrl <> 0 then
     Result := GetShiftKeyText(MapVirtualKeyW(VK_CONTROL and $FF, 0)) + '+' + Result;
end;
