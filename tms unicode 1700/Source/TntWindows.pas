...
var
  Tnt_FindFirstFileW: function(lpFileName: PWideChar; var lpFindFileData: TWIN32FindDataW): THandle; stdcall = FindFirstFile;
  Tnt_FindNextFileW: function (hFindFile: THandle; var lpFindFileData: TWIN32FindDataW): BOOL; stdcall = FindNextFile;
  Tnt_GetTempPathW: function(nBufferLength: DWORD; lpBuffer: PWideChar): DWORD; stdcall = GetTempPath;
  Tnt_SHGetFileInfoW: function(pszPath: PWideChar; dwFileAttributes: DWORD; var psfi: TSHFileInfoW; cbFileInfo, uFlags: UINT): DWORD; stdcall = SHGetFileInfo;
  Tnt_SHBrowseForFolderW: function(var lpbi: TBrowseInfoW): PItemIDList; stdcall = SHBrowseForFolder;
  Tnt_SHFileOperationW: function (const lpFileOp: TSHFileOpStructW): Integer; stdcall = SHFileOperation;

  Tnt_GetFileAttributesW: function(lpFileName: PWideChar): DWORD; stdcall = GetFileAttributes;
  Tnt_SetFileAttributesW: function(lpFileName: PWideChar; dwFileAttributes: DWORD): BOOL; stdcall = SetFileAttributes;

  Tnt_CharLowerW: function(lpsz: PWideChar): PWideChar; stdcall = CharLower;
  Tnt_CharUpperW: function(lpsz: PWideChar): PWideChar; stdcall = CharUpper;

  Tnt_DrawTextExW: function(hDC: HDC; lpString: PWideChar; nCount: Integer;
     var lpRect: TRect; uFormat: UINT;params: PDrawTextParams): Integer; stdcall = DrawTextEx;
  Tnt_DrawTextW: function(hDC: HDC; lpString: PWideChar; nCount: Integer;
     var lpRect: TRect; uFormat: UINT): Integer; stdcall = DrawText;

  Tnt_GetModuleFileNameW: function(hModule: HINST; lpFilename: PWideChar; nSize: DWORD): DWORD; stdcall = GetModuleFileName;

  Tnt_ShellExecuteW: function(hWnd: HWND; Operation, FileName, Parameters,
    Directory: PWideChar; ShowCmd: Integer): HINST; stdcall = ShellExecute;

  Tnt_LoadLibraryW: function(lpLibFileName: PWideChar): HMODULE; stdcall = LoadLibrary;

  Tnt_GetStringTypeExW: function(Locale: LCID; dwInfoType: DWORD;
    lpSrcStr: PWideChar; cchSrc: Integer; var lpCharType): BOOL; stdcall = GetStringTypeEx;

  Tnt_GetFileVersionInfoSizeW: function(lptstrFilename: PWideChar; var lpdwHandle: DWORD): DWORD; stdcall = GetFileVersionInfoSize;

  Tnt_GetFileVersionInfoW: function(lptstrFilename: PWideChar; dwHandle, dwLen: DWORD;
    lpData: Pointer): BOOL; stdcall = GetFileVersionInfo;

  Tnt_CompareStringW: function(Locale: LCID; dwCmpFlags: DWORD; lpString1: PWideChar;
    cchCount1: Integer; lpString2: PWideChar; cchCount2: Integer): Integer; stdcall = CompareString;

  Tnt_VerQueryValueW: function(pBlock: Pointer; lpSubBlock: PWideChar;
    var lplpBuffer: Pointer; var puLen: UINT): BOOL; stdcall = VerQueryValue;
...
{$IFDEF COMPILER_9_UP}
{$NODEFINE DT_NOFULLWIDTHCHARBREAK}
{$ENDIF}
...
{$IFDEF COMPILER_9_UP}
{$NODEFINE INVALID_FILE_ATTRIBUTES}
{$ENDIF}
...
function Tnt_ExtTextOutW(DC: HDC; X, Y: Integer; Options: Longint; Rect: PRect; Str: PWideChar; Count: Longint; Dx: PInteger): BOOL;
...
function Tnt_DrawTextW(hDC: HDC; lpString: PWideChar; nCount: Integer;
 ...
 {TNT-WARN DrawTextEx}
{TNT-WARN DrawTextExA}
{TNT-WARN DrawTextExW}
function Tnt_DrawTextExW(hDC: HDC; lpString: PWideChar; nCount: Integer;
  var lpRect: TRect; uFormat: UINT;params: pointer): Integer;
...
{$IFDEF COMPILER_9_UP}
{$NODEFINE PRIMARYLANGID}
{$ENDIF}
function SORTIDFROMLCID(lcid: LCID): WORD;
{$IFDEF COMPILER_9_UP}
{$NODEFINE SORTIDFROMLCID}
{$ENDIF}
...
begin
  ...
  begin
    {$IFNDEF DELPHI_UNICODE}
    s := str;
    Result := Windows.ExtTextOutA(DC, X, Y, Options, Rect, PAnsiChar(s), length(s), Dx)
    {$ENDIF}
  end;
end;
...
begin
  if Win32PlatformIsUnicode then
    Result := DrawTextExW{TNT-ALLOW DrawTextW}(hDC, lpString, nCount, lpRect, uFormat, params)
  else
    Result := DrawTextExA{TNT-ALLOW DrawTextA}(hDC,
      PAnsiChar(AnsiString(_WStr(lpString, nCount))), -1, lpRect, uFormat,params);
end;
...
function Tnt_ExtractIconW(hInst: HINST; lpszExeFileName: PWideChar;
  nIconIndex: UINT): HICON; stdcall;
begin
  if Win32PlatformIsUnicode then
    Result := ExtractIconW(hInst, lpszExeFileName, nIconIndex)
  else
    Result := ExtractIconA(hInst,
PAnsiChar(AnsiString(lpszExeFileName)), nIconIndex) end;

function Tnt_DragQueryFileW(Drop: HDROP; FileIndex: UINT; FileName:
  PWideChar; cb: UINT): UINT; stdcall;
var
  AnsiBuf: AnsiString;
  p: PAnsiChar;
begin
  if Win32PlatformIsUnicode then
    Result := DragQueryFileW(Drop, FileIndex, FileName, cb)
  else
  begin
    SetLength(AnsiBuf, MAX_PATH);
    if  FileName<>nil then p := PChar(AnsiBuf) else p := nil;
    Result := DragQueryFileA(Drop, FileIndex, p, cb);
    if FileName<>nil then WStrLCopy(FileName, PWideChar(WideString(AnsiString(p))), cb);
  end;
end;
