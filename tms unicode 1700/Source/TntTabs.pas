...
uses
  Windows, Classes, Graphics, Forms, Controls, Messages, ImgList, ComCtrls, ExtCtrls,
  TnTGraphics, TnTControls, TntClasses
...
type
  TTntScrollBtn = (sbLeft, sbRight);

  TTntScrollOrientation = (soLeftRight, soUpDown);
  ...
  TTntScroller = class(TCustomControl)
  private
    { property usage }
    FMin: Longint;
    FMax: Longint;
    ...
    FChange: Integer;
    FScrollOrientation: TTntScrollOrientation;

    { private usage }
    FBitmap: TBitmap;
    FPressed: Boolean;
    FDown: Boolean;
    FCurrent: TTntScrollBtn;
    FWidth: Integer;
    FHeight: Integer;
    ...
    procedure SetScrollOrientation(const Value: TTntScrollOrientation);
  ...
  published
    ...
    property ScrollOrientation: TTntScrollOrientation read FScrollOrientation write SetScrollOrientation;
    property Change: Integer read FChange write FChange default 1;
  end;

  TTntTabSet = class;

  TTntTabList = class(TTntStringList{TWideStringsList})
  private
    FTabSet: TTntTabSet;
  public
    constructor Create(const ATabSet: TTntTabSet);
  ...
  TTntEdgePart = (epSelectedLeft, epUnselectedLeft, epSelectedRight,
    epUnselectedRight);

  { represents the intersection between two tabs, or the edge of a tab }
  TTntEdgeType = (etNone, etFirstIsSel, etFirstNotSel, etLastIsSel, etLastNotSel,
    etNotSelToSel, etSelToNotSel, etNotSelToNotSel);

  TTntTabSetTabStyle = (tsStandard, tsOwnerDraw, tsSoftTabs, tsModernTabs, tsModernPopout);
  TTntTabStyle = TTntTabSetTabStyle;

  TTntMeasureTabEvent = procedure(Sender: TObject; Index: Integer;
    var TabWidth: Integer) of object;
  TTntTabSetDrawTabEvent = procedure(Sender: TObject; TabCanvas: TCanvas; R: TRect;
    Index: Integer; Selected: Boolean) of object;
  TTntDrawTabEvent = TTntTabSetDrawTabEvent;
  TTntTabChangeEvent = procedure(Sender: TObject; NewTab: Integer;
    var AllowChange: Boolean) of object;

  TTntTabSet = class(TCustomControl)
  private
    FStartMargin: Integer;
    FEndMargin: Integer;
    FTabs: TTntStrings; //TWideStrings;
    FTabIndex: Integer;
    FFirstIndex: Integer;
    ...
    FOnMeasureTab: TTntMeasureTabEvent;
    FOnDrawTab: TTntDrawTabEvent;
    FOnChange: TTntTabChangeEvent;
    ...
    procedure SetTabList(Value: TTntStrings);
    procedure SetTabStyle(Value: TTntTabSetTabStyle);
    ...
    procedure PaintEdge(X, Y, H: Integer; Edge: TTntEdgeType);
...
//    procedure CMHintShow(var Message: TCMHintShow); message CM_HINTSHOW;
    ...
    procedure Paint; override;
    property Scroller: TTntScroller read FScroller;
  ...
  published
    ...
    property Style: TTntTabStyle read FStyle write SetTabStyle default tsStandard;
    property TabHeight: Integer read FOwnerDrawHeight write SetTabHeight default 20;
    property Tabs: TTntStrings read FTabs write SetTabList;
    ...
    property OnClick;
    property OnChange: TTntTabChangeEvent read FOnChange write FOnChange;
    property OnDragDrop;
    property OnDragOver;
    property OnDrawTab: TTntDrawTabEvent read FOnDrawTab write FOnDrawTab;
    property OnEndDock;
    property OnEndDrag;
    property OnEnter;
    property OnExit;
    ...
    property OnMouseUp;
    property OnMeasureTab: TTntMeasureTabEvent read FOnMeasureTab write FOnMeasureTab;
...
type
  TTntTabPos = packed record
    Size, StartPos: Word;
  end;

{ TScroller }

//procedure WideCanvasTextRect(Canvas: TCanvas; Rect: TRect; Text: Widestring; TextFormat: TTextFormat = []); overload
procedure WideCanvasTextRectEx(Canvas: TCanvas; Rect: TRect; Text: Widestring; TextFormat: TTextFormat = []);
const
...
begin
  ...
  DrawTextExW(Canvas.Handle, PWideChar(Text), Length(Text), Rect, Format, nil);

  if tfModifyString in TextFormat then
    SetLength(Text, WStrLen(PWideChar(Text)));
end;

constructor TTntScroller.Create(AOwner: TComponent);
begin
...
destructor TTntScroller.Destroy;
begin
  FDownTimer.Free;
  FBitmap.Free;
  inherited Destroy;
...
procedure TTntScroller.Paint;
const
  cLeftOrient: array[TTntScrollOrientation] of string = ('TNTSBLEFT', 'TNTSBUP');               { Do not localize }
  cLeftDnOrient: array[TTntScrollOrientation] of string = ('TNTSBLEFTDN', 'TNTSBUPDN');         { Do not localize }
  cLeftDisOrient: array[TTntScrollOrientation] of string = ('TNTSBLEFTDIS', 'TNTSBUPDIS');      { Do not localize }
  cRightOrient: array[TTntScrollOrientation] of string = ('TNTSBRIGHT', 'TNTSBDOWN');           { Do not localize }
  cRightDnOrient: array[TTntScrollOrientation] of string = ('TNTSBRIGHTDN', 'TNTSBDOWNDN');     { Do not localize }
  cRightDisOrient: array[TTntScrollOrientation] of string = ('TNTSBRIGHTDIS', 'TNTSBDOWNDIS');  { Do not localize }
begin
  with Canvas do
  begin
    { paint left button }
...
procedure TTntScroller.WMSize(var Message: TWMSize);
begin
  inherited;
  Width := FWidth - 1;
  Height := FHeight;
...
procedure TTntScroller.SetMin(Value: Longint);
begin
  if Value < FMax then FMin := Value;
end;

procedure TTntScroller.SetMax(Value: Longint);
begin
  if Value > FMin then FMax := Value;
end;

procedure TTntScroller.SetPosition(Value: Longint);
begin
...
function TTntScroller.CanScrollLeft: Boolean;
begin
  Result := Position > Min;
end;

function TTntScroller.CanScrollRight: Boolean;
begin
  Result := Position < Max;
end;

procedure TTntScroller.DoMouseDown(X: Integer);
begin
...
procedure TTntScroller.WMLButtonDown(var Message: TWMLButtonDown);
begin
  DoMouseDown(Message.XPos);
end;

procedure TTntScroller.WMLButtonDblClk(var Message: TWMLButtonDblClk);
begin
  DoMouseDown(Message.XPos);
end;

procedure TTntScroller.WMMouseMove(var Message: TWMMouseMove);
var
  P: TPoint;
  R: TRect;
begin
...
procedure TTntScroller.WMLButtonUp(var Message: TWMLButtonUp);
var
  NewPos: Longint;
begin
  FDownTimer.Enabled := False;
...
procedure TTntScroller.SetScrollOrientation(const Value: TTntScrollOrientation);
const
  cWidthSize: array[TTntScrollOrientation] of Integer = (24, 26);
  cHeightSize: array[TTntScrollOrientation] of Integer = (13, 12);
begin
...
procedure TTntScroller.DoScrollTimer(Sender: TObject);
var
  NewPos: Integer;
begin
  if FDown then
...
function TTntTabList.Add(const S: Widestring): Integer;
begin
...
procedure TTntTabList.Insert(Index: Integer; const S: Widestring);
begin
...
procedure TTntTabList.Delete(Index: Integer);
var
  OldIndex: Integer;
...
procedure TTntTabList.Put(Index: Integer; const S: Widestring);
begin
...
procedure TTntTabList.AddStrings(Strings: TWideStrings);
begin
...
constructor TTntTabList.Create(const ATabSet: TTntTabSet);
begin
  inherited Create;
  FTabSet := ATabSet;
end;
...
constructor TTntTabSet.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
procedure TTntTabSet.CreateParams(var Params: TCreateParams);
begin
...
procedure TTntTabSet.CreateScroller;
begin
  FScroller := TTntScroller.Create(Self);
  with Scroller do
  begin
    Parent := Self;
    Top := 3;
...
procedure TTntTabSet.InitBitmaps;
begin
  FMemBitmap := TBitmap.Create;
  FBrushBitmap := TBitmap.Create;
end;

destructor TTntTabSet.Destroy;
begin
  FTabs.Free;
  FTabPositions.Free;
...
procedure TTntTabSet.DoneBitmaps;
begin
  FMemBitmap.Free;
  FBrushBitmap.Free;
  FEdgeImageList.Free;
...
procedure TTntTabSet.ScrollClick(Sender: TObject);
begin
  FirstIndex := TTntScroller(Sender).Position;
end;

function TabPosCompare(Item1, Item2: Pointer): Integer;
var
  FirstTabPos, SecondTabPos: TTntTabPos;
begin
  FirstTabPos := TTntTabPos(Item1);
  SecondTabPos := TTntTabPos(Item2);
  Result := SecondTabPos.Size - FirstTabPos.Size;
end;

{ cache the tab position data, and return number of visible tabs }
function TTntTabSet.CalcTabPositions(const AStart, AStop: Integer; Canvas: TCanvas;
  First: Integer; FullTabs: Boolean): Integer;
var
  Index: Integer;
  TabPos, NextTabPos: TTntTabPos;
  Start, Stop: Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      for I := 0 to J - 1 do
      begin
        TabPos := TTntTabPos(FSortedTabPositions[I]);
    ...
    begin
      ...
      MaxSize := TTntTabPos(FSortedTabPositions[0]).Size;
...
function TTntTabSet.ItemAtPos(Pos: TPoint; IgnoreTabHeight: Boolean): Integer;
var
  TabPos: TTntTabPos;
  I: Integer;
  YStart: Integer;
...
function TTntTabSet.ItemRect(Index: Integer): TRect;
var
  TabPos: TTntTabPos;
  TabTop: Integer;
begin
...
procedure TTntTabSet.DrawLine(Canvas: TCanvas; FromX, FromY, ToX, ToY: Integer);
var
  T: Integer;
begin
...
procedure TTntTabSet.DoDefaultPainting;
var
  YStart, YEnd, YMod: Integer;
  TabPos: TTntTabPos;
  Tab: Integer;
  Leading: TTntEdgeType;
  Trailing: TTntEdgeType;
...
begin
  ...
  begin
    ...
    MinRect := WideCanvasTextWidth(FMemBitmap.Canvas, 'X...'); { Do not localize }
    ...
    begin
      ...
      begin
        ...
        if (R.Right - R.Left >= MinRect) or
            (WideCanvasTextWidth(FMemBitMap.Canvas, S) <= (R.Right - R.Left)) then
//            (TextWidth(S) <= (R.Right - R.Left)) then
          WideCanvasTextRectEx(FMemBitMap.Canvas, R, S, [tfEndEllipsis, tfNoClip]);
...
procedure TTntTabSet.CreateEdgeParts;
var
  H: Integer;
  Working: TBitmap;
  EdgePart: TTntEdgePart;
  MaskColor: TColor;
  Y: Integer;
  YEnd: Integer;
  YMod: Integer;
...
begin
  ...
  begin
    ...
    try
      ...
      for EdgePart := Low(TTntEdgePart) to High(TTntEdgePart) do
      begin
...
procedure TTntTabSet.PaintEdge(X, Y, H: Integer; Edge: TTntEdgeType);
var
  TempX: Integer;
begin
  if FTabPosition in [tpLeft, tpRight] then
...
procedure TTntTabSet.CreateBrushPattern(Bitmap: TBitmap);
var
  X, Y: Integer;
begin
  Bitmap.Width := 8;
...
function TTntTabSet.ScrollerSize: Integer;
begin
  if FTabPosition in [tpTop, tpBottom] then
...
procedure TTntTabSet.FixTabPos;
var
  LastVisibleTab: Integer;
...
procedure TTntTabSet.SetSelectedColor(Value: TColor);
begin
...
procedure TTntTabSet.SetUnselectedColor(Value: TColor);
begin
...
procedure TTntTabSet.SetBackgroundColor(Value: TColor);
begin
...
procedure TTntTabSet.SetDitherBackground(Value: Boolean);
begin
...
procedure TTntTabSet.SetAutoScroll(Value: Boolean);
begin
...
procedure TTntTabSet.SetStartMargin(Value: Integer);
begin
...
procedure TTntTabSet.SetEndMargin(Value: Integer);
begin
...
function TTntTabSet.CanChange(NewIndex: Integer): Boolean;
begin
  Result := True;
...
procedure TTntTabSet.SetTabIndex(Value: Integer);
var
  F: TCustomForm;
begin
...
procedure TTntTabSet.SelectNext(Direction: Boolean);
var
  NewIndex: Integer;
begin
...
procedure TTntTabSet.SetFirstIndex(Value: Integer);
begin
...
procedure TTntTabSet.SetTabList(Value: TTntStrings);
begin
  FTabs.Assign(Value);
  FTabIndex := -1;
...
procedure TTntTabSet.SetTabStyle(Value: TTntTabStyle);
begin
...
procedure TTntTabSet.SetTabHeight(Value: Integer);
var
  SaveHeight: Integer;
begin
...
procedure TTntTabSet.DrawTab(TabCanvas: TCanvas; R: TRect; Index: Integer;
  Selected: Boolean);
begin
...
procedure TTntTabSet.GetChildren(Proc: TGetChildProc; Root: TComponent);
begin
end;

procedure TTntTabSet.MeasureTab(Index: Integer; var TabWidth: Integer);
begin
  if Assigned(FOnMeasureTab) then
    FOnMeasureTab(Self, Index, TabWidth);
end;

procedure TTntTabSet.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  I: Integer;
begin
...
procedure TTntTabSet.WMSize(var Message: TWMSize);
var
...
procedure TTntTabSet.CMSysColorChange(var Message: TMessage);
begin
  inherited;
  CreateEdgeParts;
  CreateBrushPattern(FBrushBitmap);
...
procedure TTntTabSet.CMFontChanged(var Message: TMessage);
begin
  inherited;
  Canvas.Font := Font;
  CreateEdgeParts;
...
procedure TTntTabSet.WMGetDlgCode(var Message: TWMGetDlgCode);
begin
  Message.Result := DLGC_WANTALLKEYS;
end;

procedure TTntTabSet.CMDialogChar(var Message: TCMDialogChar);
var
  I: Integer;
begin
...
function TTntTabSet.MinClientRect: TRect;
begin
  Result := MinClientRect(Tabs.Count, False);
end;

function TTntTabSet.MinClientRect(IncludeScroller: Boolean): TRect;
begin
  Result := MinClientRect(Tabs.Count, IncludeScroller);
end;

function TTntTabSet.MinClientRect(TabCount: Integer; IncludeScroller: Boolean): TRect;
var
  I: Integer;
begin
...
procedure TTntTabSet.SetFontOrientation(ACanvas: TCanvas);
var
  tf: TFont;
  lf: TLogFont;
begin
...
function TTntTabSet.ItemWidth(Index: Integer): Integer;
var
  I: Integer;
begin
  with Canvas do
  begin
    ...
    Result := WideCanvasTextWidth(Canvas, Tabs[Index]);
...
procedure TTntTabSet.SetSoftTop(const Value: Boolean);
begin
...
procedure TTntTabSet.SetImages(const Value: TCustomImageList);
begin
...
procedure TTntTabSet.ImageListChange(Sender: TObject);
begin
  Invalidate;
end;

procedure TTntTabSet.Notification(AComponent: TComponent;
...
function TTntTabSet.GetImageIndex(TabIndex: Integer): Integer;
begin
  Result := TabIndex;
...
procedure TTntTabSet.SetShrinkToFit(const Value: Boolean);
begin
...
function TTntTabSet.GetHint: WideString;
begin
  Result := TntControl_GetHint(Self)
end;

procedure TTntTabSet.SetHint(const Value: WideString);
begin
  TntControl_SetHint(Self, Value);
end;

function TTntTabSet.IsHintStored: Boolean;
begin
  Result := TntControl_IsHintStored(Self)
end;

procedure TTntTabSet.CMHintShow(var Message: TMessage);
//procedure TTntTabSet.CMHintShow(var Message: TCMHintShow);
(*var
  R, Calced: TRect;
  I: Integer;
  S: Widestring;
  ...
  if FShrinkToFit and (Message.HintInfo.HintControl = Self) then
    ...
    begin
      ...
      begin
        ...
        Calced := R;
        S := FTabs[I];
        //procedure WideCanvasTextRectX(Canvas: TCanvas; Rect: TRect; var Text: string; TextFormat: TTextFormat = []);
        WideCanvasTextRect(Canvas, Calced, S, [tfCalcRect]);
//        WideCanvasTextRectX(Canvas, Calced, S, [tfCalcRect]);
...
procedure TTntTabSet.SetTabPosition(const Value: TTabPosition);
begin
...
procedure TTntTabSet.DoModernPainting;
var
  YStart: Integer;
  TabPos: TTntTabPos;
  Tab: Integer;
...
procedure TTntTabSet.DoPopoutModernPainting;
var
  TabPos: TTntTabPos;
  Tab: Integer;
  isSelected: Boolean;
  ...
  begin
    ...
    begin
      ...
      if (TabRect.Right - TabRect.Left >= MinRect) or
          (WideCanvasTextWidth(FMemBitmap.Canvas, S) <= (TabRect.Right - TabRect.Left)) then
//          (TextWidth(S) <= (TabRect.Right - TabRect.Left)) then
        WideCanvasTextRectEx(FMemBitmap.Canvas, TabRect, S, [tfEndEllipsis, tfNoClip]);
//        TextRect(TabRect, S, [tfEndEllipsis, tfNoClip]);
    end;
  end;
end;

procedure TTntTabSet.SetupTabPositions;
var
...
function TTntTabSet.ScrollerShown: Boolean;
begin
  Result := AutoScroll and (FVisibleTabs < Tabs.Count);
end;

procedure TTntTabSet.WMEraseBkgnd(var Message: TWMEraseBkgnd);
begin
  if ThemeServices.ThemesEnabled and ParentBackground then
    inherited
  else
