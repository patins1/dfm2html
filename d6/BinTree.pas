...
type
  ...
  protected
    procedure FindLE(Compare: TListSortCompare; Item1: Pointer;
      var Eq: boolean; var Index: PTreeNode; NeedSamePointer:boolean; var Where:TChildPos; NeedLeaf:boolean);
  public
    destructor Destroy; override;
    procedure Clear;
    function GetTreeNode(Compare: TListSortCompare; Item1: Pointer): PTreeNode;
...
//returns true if item was already contained
function TBinTree.AddItem(Compare: TListSortCompare;
  Item1: Pointer): boolean;
var Index:PTreeNode;
var Where:TChildPos;
begin
 FindLE(Compare,Item1,result,Index,false,Where,true);//because of NeedLeaf=true, stability is given, i.e. equal-weighted items appear in the tree in the same order as they are added with AddItem
...
var Where:TChildPos;
begin
 FindLE(Compare,Item1,result,Index,false,Where,false);
 if not result then
  Insert(Index,Item1,Where);
end;
...
function TBinTree.GetTreeNode(Compare: TListSortCompare;
  Item1: Pointer): PTreeNode;
var Where:TChildPos;
    Found:boolean;
begin
 FindLE(Compare,Item1,Found,Result,true,Where,false);
end;
