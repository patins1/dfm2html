...
type
  TTntCustomIniFile = TCustomIniFile;

  TTntMemIniFile = TMemIniFile;
...
type
  ...
  public
    ...
    procedure ReadSections(Strings: TTntStrings); overload; virtual; abstract;
    procedure ReadSections(const Section: WideString; Strings: TTntStrings); overload; virtual;
  ...
  { TTntMemIniFile - loads an entire INI file into memory and allows all
...
procedure TTntCustomIniFile.ReadSections(const Section: WideString;
  Strings: TTntStrings);
var
  I: Integer;
begin
  ...
  for I := Strings.Count - 1 downto 0 do
    if not WideSameText(Section, Copy(Strings[I], 1, Length(Section))) then
      Strings.Delete(I);
end;
...
constructor TTntStringHash.Create(Size: Cardinal);
begin
  inherited Create;
      ...
      begin
        ...
        begin
          ...
          if J > 0 then // remove spaces before and after NameValueSeparator
            Strings.Add(Trim(Copy(S, 1, J-1)) + FSections.NameValueSeparator + Copy(S, J+1, MaxInt))
          else
            Strings.Add(S);
        end;
  end;
...
{.$DEFINE FixWritePrivateProfileStringW}

{$IFDEF FixWritePrivateProfileStringW}
// If the file path contains unicode chars and the path has not been created 
// completely. The unicode path will not be created automatically. 
// To note that WritePrivateProfileStringW() will always create ANSI-encoded
// files. I am consider of extend the patch.
procedure FixWritePrivateProfileStringW(const AFileName: WideString);
var
  Dir: WideString;
begin
  Dir := WideExtractFilePath(AFileName);
  if (Dir <> '') then
    WideForceDirectories(Dir);
end;
{$ENDIF}
...
begin
  if (not Win32PlatformIsUnicode) then
    ...
    { Windows NT/2000/XP and later }
  {$IFDEF FixWritePrivateProfileStringW}
    FixWritePrivateProfileStringW(FileName);
  {$ENDIF}
