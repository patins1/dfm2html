...
begin
    ...
    FConnected := TRUE;//°!°bei internem Redirect
...
//            if Copy(FLocation, 1, 6) <> 'ftp://' then //°!°
                FCtrlSocket.OnSessionClosed := LocationSessionClosed{ else
            begin
            FCtrlSocket.OnSessionClosed := SocketSessionClosed;
            FLocationFlag:=false;
    ...
    if (FHeaderLineCount = 1) and
       ((Copy(FLastResponse, 1, 8) = 'HTTP/1.0') or
        (Copy(FLastResponse, 1, 8) = 'RTSP/1.0') or //°!°
        ...
        if Field = 'location' then begin { Change the URL ! }
            ...
            if Copy(Data, 1, 6) = 'ftp://' then begin //°!°
             FLocationFlag := TRUE;
             FLocation := Data;
             FCurrentProtocol:='ftp'; //
{
             if Assigned(FOnLocationChange) then
              FOnLocationChange(Self);
}
            end else begin
            Data:=StringReplace(Data,' ','%20',[rfReplaceAll,rfIgnoreCase]); //°!°  sh. MapHTTPURL
...
begin
 ...
 {   for i:=FReceiveLen+Len+1 to SizeOf(FReceiveBuffer)-1 do
     FReceiveBuffer[i]:='X';
    setstring(Sss,pchar(@FReceiveBuffer[0]),FReceiveLen+Len+5);
    if Sss='FASD' then exit;
      }
